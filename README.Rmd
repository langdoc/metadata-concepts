...
So more recording sessions we have to exclude from comparison because the metadata field in question is lacking, more peripheral that field has to be. Someone can argue it is super-important, but if nobody has never collected it, then it probably is not. Another example is that for M.A. Castrén's data we can't really specify the participants or locations, whereas for bit newer data such as that of Fokos-Fuchs's we generally can do that, up to a certain degree, so this sets kind of a central boundary between these two datasets.
